...
func (h *BaseHandler) HandleSelectServer(c telebot.Context) error {
	// Get server names
	serverNames := h.xrayService.GetServerNames()

	// Create keyboard with server names
	markup := &telebot.ReplyMarkup{
		ResizeKeyboard: true,
	}
	...
	// Set state to awaiting server selection
	err := h.stateService.WithConversationState(c.Sender().ID, models.AwaitingSelectServer)
	if err != nil {
		h.logger.Errorf("Failed to set state: %v", err)
		return err
	}

	// Send message with server selection keyboard
	return h.sendTextMessage(c, "Please select a server:", markup)
}
